...
`;console.log(l());const m=l().key,y=await h("./public/prompts/template.txt");console.log(y);const v=await f("./public/prompts/character.json");console.log(v);document.querySelector("#counter").addEventListener("click",function(){g(m,document.getElementById("messageInput").value,"onyx").then(o=>{o?new Audio(o).play():console.error("Failed to get speech URL")}).catch(o=>console.error(o))});const i=document.getElementById("recordButton");let a=!1;function b(){console.log("録音が開始されました。")}async function w(){return console.log("録音が停止しました。"),new Blob}i.addEventListener("click",async()=>{if(!a)b(),i.classList.add("recording"),a=!0;else{const o=await w();i.classList.remove("recording"),a=!1,console.log("録音データ:",o)}});
