...
`;console.log(i());const h=i().key,g=await p("./prompts/template.txt");console.log(g);const m=await d("./prompts/character.json");console.log(m);document.querySelector("#counter").addEventListener("click",function(){f(h,"私はクワガタ型メダロットのロクショウです。","onyx").then(r=>{r?new Audio(r).play():console.error("Failed to get speech URL")}).catch(r=>console.error(r))});
